...
class CombinedVisualizer:
    ...
    def extract_sentiment_and_stock_features(self, return_as_dict=False):
        ...
        for date_key, feature_dict in self.stock_feature_dict.items():
            ...
            # extract stock and social media features
                ...
                twitter_predictions_list_help = self.twitter_feature_dict.get(date_key).get(
                    GlobalConfig.PREDICTION_LIST)
        ...
        if return_as_dict:
            ...
            stock_social_media_dict.update(
                {GlobalConfig.TWITTER_AVERAGE_SENTIMENT: np.asarray(twitter_avg_sentiment_list)})
            ...
            stock_social_media_dict.update(
                {GlobalConfig.TWITTER_MAX_SENTIMENT_MARGIN: np.asarray(twitter_max_sentiment_margin_list)})
            stock_social_media_dict.update(
                {GlobalConfig.REDDIT_AVERAGE_SENTIMENT: np.asarray(reddit_avg_sentiment_list)})
            ...
            stock_social_media_dict.update(
                {GlobalConfig.REDDIT_MAX_SENTIMENT_MARGIN: np.asarray(reddit_max_sentiment_margin_list)})
    ...
    def plot_sentiment_stock_feature_graphics(self,
        ...
        over_night_diff_trace = go.Scattergl(x=sentiment_stock_feature_dict[GlobalConfig.DATE_LIST],
                                             ...
                                             text='Price difference from close (yesterday) '
                                                  'to open (today) - rescaled to [-1; 1]',
                                             showlegend=True,
                                             hoverinfo='text',
                                             opacity=0.7,
        ...
        per_change_trace = go.Scattergl(x=sentiment_stock_feature_dict[GlobalConfig.DATE_LIST],
                                        ...
                                        text='Percentage price change between open '
                                             'and close today - rescaled to [-1; 1] ',
                                        showlegend=True,
                                        hoverinfo='text',
                                        opacity=0.7,
                                        hoverlabel=dict(bgcolor='grey'))
        ...
        vol_fluc_trace = go.Scattergl(x=sentiment_stock_feature_dict[GlobalConfig.DATE_LIST],
                                      ...
                                      text='Volume standard deviation between open '
                                           'and close today - rescaled to [-1; 1] ',
                                      showlegend=True,
                                      hoverinfo='text',
                                      opacity=0.7,
    ...
    def plot_correlation_bar_chart(self, twitter_feature=GlobalConfig.TWITTER_NUM_POS,
        ...
        reddit_corrcoef_list.append(
            np.corrcoef(sentiment_stock_feature_dict[reddit_feature], over_night_diff_list_rescaled)[0, 1])
        reddit_corrcoef_list.append(
            np.corrcoef(sentiment_stock_feature_dict[reddit_feature], max_margin_list_rescaled)[0, 1])
        reddit_corrcoef_list.append(
            np.corrcoef(sentiment_stock_feature_dict[reddit_feature], abs_diff_list_rescaled)[0, 1])
        reddit_corrcoef_list.append(
            np.corrcoef(sentiment_stock_feature_dict[reddit_feature], per_change_list_rescaled)[0, 1])
        reddit_corrcoef_list.append(
            np.corrcoef(sentiment_stock_feature_dict[reddit_feature], abs_vol_list_rescaled)[0, 1])
        reddit_corrcoef_list.append(
        ...
        twitter_corrcoef_list.append(
            np.corrcoef(sentiment_stock_feature_dict[twitter_feature], max_margin_list_rescaled)[0, 1])
        twitter_corrcoef_list.append(
            np.corrcoef(sentiment_stock_feature_dict[twitter_feature], abs_diff_list_rescaled)[0, 1])
        twitter_corrcoef_list.append(
            np.corrcoef(sentiment_stock_feature_dict[twitter_feature], per_change_list_rescaled)[0, 1])
        twitter_corrcoef_list.append(
            np.corrcoef(sentiment_stock_feature_dict[twitter_feature], abs_vol_list_rescaled)[0, 1])
        twitter_corrcoef_list.append(
            np.corrcoef(sentiment_stock_feature_dict[twitter_feature], vol_fluc_list_rescaled)[0, 1])
        twitter_corrcoef_list.append(
            np.corrcoef(sentiment_stock_feature_dict[twitter_feature], price_fluc_list_rescaled)[0, 1])
    ...
    def plot_distribution_comparison(self,
        ...
        # plot histograms
        plt.hist(stock_feature_list_rescaled, bins=25, alpha=0.2, color='green', label=stock_feature)
        plt.hist(twitter_feature_list_rescaled, bins=25, alpha=0.2, color='blue', label=twitter_feature)
        plt.hist(reddit_feature_list_rescaled, bins=25, alpha=0.2, color='red', label=reddit_feature)
    ...
    @staticmethod
    def rescale_array(x, lower_bound, upper_bound):
